...
public partial class SupportCenter_Categories : CustomerSupportPage
{
    public string strCategoryName = "Explore Categories";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            BindMainTopicCategories();

            divAllCategoriesHeader.Visible= true;
            divSpecificCategoriesHeader.Visible = false;
            rptrMainTopicCategories.Visible = true;
            rptrRightTopicCategories.Visible = false;
            Page.Title = "Explore by Categories" + " : " + GetState(CustomerSupportPage.Account_Name);
               
        }
        else
        {
            BindRightTopicCategories();
            BindLeads();
            divAllCategoriesHeader.Visible = false;
            divSpecificCategoriesHeader.Visible = true;
            rptrMainTopicCategories.Visible = false;
            rptrRightTopicCategories.Visible = true;
            Page.Title = strCategoryName + " : " + GetState(CustomerSupportPage.Account_Name);
               
        }
        
        
    }
    private void BindMainTopicCategories()
    {
        ...
        rptrMainTopicCategories.DataSource = objCategoryTopicsDataset.Tables[0];
        rptrMainTopicCategories.DataBind();
        if (objCategoryTopicsDataset.Tables[0].Rows.Count == 0)
        {
    ...
    private void BindRightTopicCategories()
    {
        CategoryTopics objCategoryTopics = new CategoryTopics();

        DataSet objCategoryTopicsDataset;
        objCategoryTopicsDataset = objCategoryTopics.GetCategoryTopics(GetState(CustomerSupportPage.AccountID));
        rptrRightTopicCategories.DataSource = objCategoryTopicsDataset.Tables[0];
        rptrRightTopicCategories.DataBind();
